...
describe("buildTextTurnParams", () => {
  ...
  it("includes the organization handle in metadata", () => {
    const params = buildTextTurnParams("Hi", {
      handle: "my-brand",
    });
    expect(params.metadata?.handle).toBe("my-brand");
  });

  it("does not emit an unconfigured public identity", () => {
    const params = buildTextTurnParams("Hi");
    expect(params.metadata?.handle).toBeUndefined();
  });
});
