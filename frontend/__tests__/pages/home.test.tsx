...
// The home page renders its sections from i18n `home.*` keys plus a blog
...
describe("Home Page", () => {
  it("renders a single h1 from the hero headline", async () => {
    render(await renderHome());

    const headings = screen.getAllByRole("heading", { level: 1 });
    expect(headings).toHaveLength(1);
    expect(headings[0]).toHaveTextContent(en.home.hero.headline);
  });

  it("renders the hero subhead", async () => {
    render(await renderHome());

    expect(screen.getByText(en.home.hero.subhead)).toBeInTheDocument();
  });
