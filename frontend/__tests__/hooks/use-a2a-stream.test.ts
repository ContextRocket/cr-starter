...
describe("useA2AStream", () => {
  ...
  describe("direct A2A identity and credential shaping", () => {
    it("includes the organization handle in metadata", async () => {
      ...
      expect(body.params.metadata?.handle).toBe("my-brand");
      expect(fetchSpy.mock.calls[0][1].headers["X-Api-Key"]).toBe(
        "publishable-key",
      );
    });

    it("does not emit a legacy public identity field", async () => {
      const fetchSpy = vi.fn().mockResolvedValueOnce({
        ok: true,
        body: makeSSEBody([
          {
    ...
    it("handle errors surface a resolved string", async () => {
      mockStreamFetch([
        {
          type: "TaskStatusUpdateEvent",
          id: "td4",
          ...
          metadata: {
            customerSafe: true,
            error_key: "no published content for handle 'bad-id'",
            reason: "no published content for handle 'bad-id'",
          },
        },
      ]);

      const { result } = renderHook(() =>
        useA2AStream({ baseUrl: AGENT_URL, handle: "bad-id" }),
      );
      ...
      // The error message must NOT be a raw i18n key.
