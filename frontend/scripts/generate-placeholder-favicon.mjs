...
console.log(`Generated placeholder SVG favicon at ${outPath} with color ${color} and letter '${letter}'`);

// Wait to resolve sharp so it plays well in ESM
(async () => {
  const sharp = (await import('sharp')).default;
  await sharp(outPath).resize(512, 512).png().toFile(path.resolve(__dirname, '../public/favicon-source.png'));
  console.log("Converted SVG to PNG. You can now run pnpm run generate-favicons.");
})();
