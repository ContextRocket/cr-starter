...
test.describe("JSON-LD structured data", () => {
  ...
  test("a blog post emits BlogPosting + BreadcrumbList JSON-LD", async ({
    ...
    // Full navigation (not a client-side click) so the server-rendered
    // JSON-LD scripts for the post are present before we read them.
    const href = await firstPost.getAttribute("href");
    await page.goto(href ?? "/en/blog", { waitUntil: "domcontentloaded" });
