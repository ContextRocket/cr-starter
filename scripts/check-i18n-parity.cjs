 ...
 *   - All locales defined in site.config.ts have corresponding message files
 *   - All message categories (app, shared, site) have files for each locale
 *
 * Usage:
 ...
 *         failed to parse, or missing message files
 */
...
const MESSAGES_DIR = (() => {
  const candidate1 = path.resolve(__dirname, "../src/i18n/messages");
  const candidate2 = path.resolve(__dirname, "../i18n/messages");
...
/** Discover locale barrels instead of freezing the starter to en/es/de. */
const SUPPORTED_LOCALES = fs
  .readdirSync(MESSAGES_DIR)
  .map((name) => name.match(/^([a-z]{2,3}(?:-[A-Z][a-z]{2})?)\.ts$/)?.[1])
  .filter(Boolean)
  .sort();

// ---------------------------------------------------------------------------
// Verify site.config.ts locales have message files
// ---------------------------------------------------------------------------

/**
 * Read site.json / site.config and extract the locales array.
 * @returns {string[]} The locales defined for this site
 */
function getSiteConfigLocales() {
  const candidates = [
    // Astro flat layout: config/ at repo root (scripts/ -> ../config)
    path.resolve(__dirname, "../config/site.json"),
    // From messages dir: src/i18n/messages -> ../../../config
    path.resolve(MESSAGES_DIR, "../../../config/site.json"),
    // Legacy Next layout: frontend/config
    path.resolve(MESSAGES_DIR, "../../config/site.json"),
  ];
  for (const siteJsonPath of candidates) {
    if (!fs.existsSync(siteJsonPath)) continue;
    try {
      const siteData = JSON.parse(fs.readFileSync(siteJsonPath, "utf8"));
      if (Array.isArray(siteData.locales)) return siteData.locales;
    } catch (err) {
      console.warn(`WARNING: Could not parse ${siteJsonPath}: ${err.message}`);
    }
  }

  const siteConfigCandidates = [
    path.resolve(__dirname, "../src/config/site.config.ts"),
    path.resolve(MESSAGES_DIR, "../../config/site.config.ts"),
  ];
  const siteConfigPath = siteConfigCandidates.find((p) => fs.existsSync(p));
  if (!siteConfigPath) {
    console.warn(
      "WARNING: site.json / site.config.ts not found, skipping locales verification",
    );
    return [];
  }
  ...
  try {
    ...
    console.warn(`WARNING: Could not parse site.config.ts: ${err.message}`);
    return [];
  }
}

const SITE_LOCALES = getSiteConfigLocales();
// Only parity-check locales the site actually serves. English-only forks may
// keep unused shared/app es/de barrels from parent sync without failing the gate.
const CHECK_LOCALES = SUPPORTED_LOCALES.filter((l) => {
  if (l === REFERENCE_LOCALE) return false;
  if (SITE_LOCALES.length > 0 && !SITE_LOCALES.includes(l)) return false;
  return true;
});
if (SITE_LOCALES.length > 0) {
  const skipped = SUPPORTED_LOCALES.filter(
    (l) => l !== REFERENCE_LOCALE && !SITE_LOCALES.includes(l),
  );
  if (skipped.length > 0) {
    console.log(
      `Skipping unused locale barrels (not in site.json locales): ${skipped.join(", ")}`,
    );
  }
}

/**
 * Verify that all locales in site.config.ts have corresponding message files.
 * @returns {boolean} true if all locales have message files
 */
function verifyLocalesHaveMessageFiles() {
  const siteConfigLocales = getSiteConfigLocales();
  if (siteConfigLocales.length === 0) return true;

  let hasError = false;
  const missingFiles = [];

  for (const locale of siteConfigLocales) {
    for (const domain of DOMAIN_ORDER) {
      const slicePath = path.join(MESSAGES_DIR, domain, `${locale}.ts`);
      if (!fs.existsSync(slicePath)) {
        missingFiles.push(`${domain}/${locale}.ts`);
        hasError = true;
      }
    }
  }
...
// barrel (<locale>.ts) is just a spread-merge of these slices, and the app
// slice is further decomposed into per-surface files under modules/ (spread
// back into appEn). We collect leaves from the slices + modules surfaces
// directly so parity sees the real object literals.
const DOMAIN_ORDER = ["shared", "app", "site"];

/**
 * Collect every leaf dot-path from the per-surface modules/<surface>/<locale>.ts
 * files. Auto-discovers surface subdirectories (dev, chat, embed, ...); a
 * missing locale file for a surface simply contributes nothing.
 *
 * @param {string} locale
 * @returns {{ path: string, kind: "string"|"array" }[]}
 */
function collectModulesLeaves(locale) {
  const out = [];
  const modulesDir = path.join(MESSAGES_DIR, "modules");
  if (!fs.existsSync(modulesDir)) return out;

  function visit(dir) {
    for (const entry of fs.readdirSync(dir, { withFileTypes: true })) {
      if (!entry.isDirectory()) continue;
      const surfaceDir = path.join(dir, entry.name);
      const surfaceFile = path.join(surfaceDir, `${locale}.ts`);
      if (fs.existsSync(surfaceFile)) {
        out.push(...collectLeaves(loadObjectFromFile(surfaceFile)));
      }
      visit(surfaceDir);
    }
  }
 ...
 * plus the carved modules/ surfaces.
 *
 * @param {string} locale
 * @returns {{ path: string, kind: "string"|"array" }[]}
 */
function collectLocaleLeaves(locale) {
  ...
  out.push(...collectModulesLeaves(locale));
  return out;
}
...
// First verify that all locales in site.config.ts have message files
if (!verifyLocalesHaveMessageFiles()) {
  process.exit(1);
}
