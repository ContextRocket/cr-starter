 ...
 *     either section.
 *
 * BUILD-TIME ONLY: fs/path imports are Node-only. Use from Astro pages or
 * loaders; never import from a client island.
 */
...
/**
 * Resolve content/attributions.json from the Astro repo root (process.cwd()).
 */
function resolveAttributionsPath(): string {
  const fromRoot = path.resolve(process.cwd(), "content/attributions.json");
  if (fs.existsSync(fromRoot)) return fromRoot;

  // Fallback when tooling runs with a nested cwd.
  const fromParent = path.resolve(
    process.cwd(),
    "..",
    "content/attributions.json",
  );
